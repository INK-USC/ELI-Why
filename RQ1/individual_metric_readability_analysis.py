...
def main(model_name: str, set_type: str):
    ...
    if not os.path.exists(classification_csv):
        ...
        all_labels = []
        for question, explanation in tqdm(zip(all_questions, all_explanations), total=len(all_questions)):
            all_labels.append(classify_explanation(question, explanation))

        classification_df = pd.DataFrame(index=data.index)
        classification_df['Question'] = data['Question']
        for label, idx, role in zip(all_labels, all_indices, all_roles):
            classification_df.at[idx, role] = label
